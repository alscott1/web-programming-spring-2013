...
if ($genres = $mysqli->query("SELECT * from genres2")) {
  while ($genre = $genres->fetch_object())
  {
   $musicarray[$genre->idgenres] = $genre;
   if ($artists = $mysqli->query("select idartists , artist from artists2 where genres_idgenres = ".$genre->idgenres)) {
while ($artist = $artists->fetch_object())
{
$musicarray[$genre->idgenres]->artists[$artist->idartists] = $artist;
if ($albums = $mysqli->query("select idalbums , album from albums2 where artists_idartists = ".$artist->idartists)) {
while ($album = $albums->fetch_object())
{
$musicarray[$genre->idgenres]->artists[$artist->idartists]->albums[$album->idalbums] = $album;
if ($tracks = $mysqli->query("select * from tracks2 where albums_idalbums = ".$album->idalbums)) {
